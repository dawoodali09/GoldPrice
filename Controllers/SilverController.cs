...
public class SilverController : Controller
{
    ...
    public SilverController(GoldPriceDbContext dbContext, IGoldApiService goldApiService)
    {
    ...
    {
        var viewModel = await GetMetalViewModel("Silver");
    ...
    {
        var result = await _goldApiService.FetchLatestPrice("Silver");
        if (result == null)
        {
            TempData["Error"] = "Failed to fetch silver price. Please try again later.";
        }
        else
        {
            TempData["Success"] = $"Silver price updated: {result.PricePerGram:N2} NZD/gram";
        }
    ...
    public async Task<IActionResult> AddHolding(decimal quantity, decimal? purchasePrice, DateTime? purchaseDate, string? description)
    {
        ...
        TempData["Success"] = "Silver holding added successfully.";
    ...
    public async Task<IActionResult> EditHolding(int id, decimal quantity, decimal? purchasePrice, DateTime? purchaseDate, string? description)
    {
        ...
        if (holding != null && holding.MetalType == "Silver")
        {
